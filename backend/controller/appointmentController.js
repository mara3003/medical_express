...
export const countAppointments = catchAsyncErrors(async(req, res, next)=>{
    ...
    try {
      // Folosește metoda `countDocuments` pentru a număra appointments
      const count = await Appointment.countDocuments();
      // Returnează numărul de appointments într-un obiect JSON
      res.status(200).json({ count });
    } catch (error) {
      // În caz de eroare, trimite un răspuns cu statusul 500 și mesajul de eroare
